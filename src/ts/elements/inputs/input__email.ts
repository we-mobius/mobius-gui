...
export type EmailInputValue = EmailInputorValue

export const makeEmailInputE = createElementMaker<EmailInputElementOptions>({
  marks: {},
  styles: {
    type: 'EmailInput',
    name: '',
  ...
  prepareTemplate: (view, { styles, actuations, utils: { html } }) => {
    ...
    if (direction === 'horizontal') {
      const { name, label, title, description, information, instructions, value, minlength, maxlength, multiple, placeholder } = styles
      ...
      return makeFormItemLayoutE({
        styles: {
          ...
          input: makeEmailInputorE({
            styles: { name, title, value, minlength, maxlength, multiple, placeholder },
